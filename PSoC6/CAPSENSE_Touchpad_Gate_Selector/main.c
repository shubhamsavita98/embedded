...
*              CapSense buttons. Using touch slider we select the logic gates
*              and the buttons works accordingly. Real-time slider position is
*              shown on the UART console whenever the slider position changes.
*              The code utilizes interrupts.It also includes CAPSENSE Tuner
*              GUI implementation from CAPSENSE default example.
*
*******************************************************************************
...
static void process_touch(void)
{
    ...
    /* Select gate: AND, OR, XOR */
	led_update_req = true;
	if(slider_pos < 100 ){ // OR
		if ((button0_status) || (button1_status))
		{
	...
	else if	((slider_pos > 100) && (slider_pos < 200)){ // AND
		if ((button0_status) && (button1_status))
		{
			led_data.state = LED_ON;
			led_update_req = true;
	...
	else if ((slider_pos > 200) && (slider_pos < 300)){ // XOR
		if ((button0_status) ^ (button1_status))
		{
			led_data.state = LED_ON;
			led_update_req = true;
...
static uint32_t initialize_capsense(void)
{
    ...
    status = Cy_CapSense_Init(&cy_capsense_context);// needs help
    ...
    status = Cy_CapSense_Enable(&cy_capsense_context);// needs help
    if (CYRET_SUCCESS != status)
    {
        return status;
    }
